...
## 🟩 Phase 7: Factory Flash Manifest & Guided Burning (工厂烧录清单与引导烧录)
Make PioPulse able to consume a factory flashing manifest so operators do not manually map files to flash offsets.

- [x] **ProjectConfig Factory Manifest Support**:
  - [x] Add a structured firmware image list to `ProjectConfig`, with fields for `label`, `path`, `offset`, `required`, and `sha256`.
  - [x] Support both segmented flashing and merged-bin flashing modes.
  - [x] Persist ESP flash parameters in config: `chip_type`, `flash_size`, `flash_mode`, `flash_freq`, and `do_not_chg_bin`.
- [x] **PixelPad Factory Package Compatibility**:
  - [x] Import `factory/project_config.json` style packages generated by PixelPad.
  - [x] Recognize this segmented layout by default: `0x0000 -> bootloader.bin`, `0x8000 -> partitions.bin`, `0xe000 -> boot_app0.bin`, `0x10000 -> firmware.bin`.
  - [x] Recognize merged layout: `0x0000 -> factory_merged.bin`.
  - [x] Display `DoNotChgBin = true` prominently and avoid mutating bin headers when it is enabled.
- [x] **Guided Burning UI**:
  - [x] Show a read-only table of flash offset, filename, file size, sha256 status, and required/optional state before flashing.
  - [x] Add operator-friendly validation errors for missing files, duplicate offsets, invalid hex offsets, mismatched sha256, and unsupported chip type.
  - [x] Add a one-click switch between segmented flashing and merged-bin flashing when both are available.
- [x] **Flashing Backend Integration**:
  - [x] Convert the manifest into the esptool write command without requiring manual file selection.
  - [x] Pass flash parameters explicitly when the backend needs them, but never rewrite image headers when `do_not_chg_bin` is true.
  - [x] Add unit tests for manifest parsing, offset validation, sha256 validation, and command generation.
